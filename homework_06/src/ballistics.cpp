...
double calculate_horizontal_range_by_time(double V0, double Z0, double d, double m, double l, double t) {
    ...
    double h = V0 * t - t_2 * d * V0 / (2 * m)
             + t_3 * (6 * d * G * l * m - 6 * d_2 * (l_2 - 1) * V0) / (36 * m_2)
             + t_4 * (-6 * d_2 * G * l * (1 + l_2 + l_4) * m + 3 * d_3 * l_2 *(1 + l_2) * V0 + 6 * d_3 * l_4 * (1 + l_2) * V0) / (36 * (1 + l_2) * (1 + l_2) * m_3)
             + t_5 * (3 * d_3 * G * l_3 * m - 3 * d_4 * l_2 * (1 + l_2) * V0) / (36 * (1 + l_2) * m_4);
    
    return h;
}

bool compute_horizontal_range(
    double attackSpeed,
    double zd,
    double d,
    double l,
...
bool read_input_file(const char* path, double& xd, double& yd, double& zd, double& target_x, double& target_y, double& attack_speed, double& acceleration_path, std::string& ammo_name){
